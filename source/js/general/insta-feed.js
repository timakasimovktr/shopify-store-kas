const $feedHolder = $('#instafeed');

if ($feedHolder.length) {
  const access_Token = $feedHolder.data('access_token');
  const column_cnts = $feedHolder.data('columns');
  let resolution = "standard_resolution";


  (function(){var Instafeed=(function(){function Instafeed(params,context){var option,cachedDomains=['nuno-shop.myshopify','www.madsnorgaard.dk'],value;this.options={target:'instafeed',title:'',columns:5,links:true,mock:false,useHttp:false};if(typeof params==='object'){for(option in params){value=params[option];this.options[option]=value;}}
  ...
  if((this.options.template!==null)&&typeof this.options.template==='string'){htmlString='';imageString='';imgUrl='';tmpEl=document.createElement('div');var sizeNeeded=(targetEl.clientWidth/this.options.columns)*window.devicePixelRatio
  resolution="standard_resolution";if(sizeNeeded<=150){resolution="thumbnail";}else if(sizeNeeded<=320){resolution="low_resolution";}
  for(i=0,len=images.length;i<len;i++){image=images[i];imageObj=image.images[resolution];if(typeof imageObj!=='object'){eMsg="No image found for resolution: "+resolution+".";throw new Error(eMsg);}
  ...
  for(j=0,len1=childNodesArr.length;j<len1;j++){node=childNodesArr[j];fragment.appendChild(node);}}
  if(this.options.title.length>0){var elChild=document.createElement('h2');elChild.innerHTML=this.options.title;targetEl.insertBefore(elChild,targetEl.firstChild);}
  ...
  return true;};Instafeed.prototype._buildUrl=function(){var base,endpoint,final;base="https://api.instagram.com/v1";if(window.location.hostname==='rex-kralj-outlet.com'||window.location.hostname==='www.madsnorgaard.dk'||window.location.hostname==='www.madsnorgaard.com'||window.location.hostname==='nuno-shop.myshopify.com'){base="https://instafeed.n3f.me/index/v1";}
  endpoint="users/self/media/recent";final=base+"/"+endpoint;final+="?access_token="+this.options.accessToken;final+="&count="+this.options.limit;final+="&callback=instafeedCache"+this.unique+".parse";return final;};Instafeed.prototype._genKey=function(){var S4;S4=function(){return(((1+Math.random())*0x10000)|0).toString(16).substring(1);};return""+(S4())+(S4())+(S4())+(S4());};Instafeed.prototype._makeTemplate=function(template,data){var output,pattern,ref,varName,varValue;pattern=/(?:\{{2})([\w\[\]\.]+)(?:\}{2})/;output=template;while(pattern.test(output)){varName=output.match(pattern)[1];varValue=(ref=this._getObjectProperty(data,varName))!==null?ref:'';output=output.replace(pattern,function(){return""+varValue;});}
  ...
  return object;};Instafeed.prototype._filter=function(images,filter){var filteredImages,fn,i,image,len;filteredImages=[];fn=function(image){if(filter(image)){return filteredImages.push(image);}};for(i=0,len=images.length;i<len;i++){image=images[i];fn(image);}
  return filteredImages;};return Instafeed;})();function escapeHtml(unsafe){if(unsafe){unsafe=unsafe.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#039;");}
  return unsafe;}
  (function(root,factory){if(typeof define==='function'&&define.amd){return define([],factory);}else if(typeof module==='object'&&module.exports){module.exports=factory();return module.exports;}else{root.Instafeed=factory();return root.Instafeed;}})(this,function(){return Instafeed;});if(document.getElementById('instafeed')!==null){var feed=new Instafeed({title:'',columns:column_cnts,limit:column_cnts,accessToken:access_Token,template:'<a href="{{link}}" target="_blank"><div class="container" style="width:20%;padding-top:20%;"><img style="width:99%;height:99%;" class="lazyload" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" data-src="{{image}}" alt="Kassatex Instagram" /><div style="width:99%;height:99%;" class="overlay"></div></div></div></a>'});feed.run();}})();
}